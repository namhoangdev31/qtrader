...
async def test_approval_pipeline_fsm_failure() -> None:
    bus = AsyncMock()
    fsm = AsyncMock()
    fsm.transition.return_value = False
    ...
    sandbox_report = SandboxReportEvent(
        ...
        timestamp=1,
        payload=SandboxReportPayload(
            strategy_id=STRATEGY_ID, pnl=2.0, drawdown=0.01, sharpe=1.0, status="S", trade_count=1
        ),
    )
    risk_event = ModelRiskScoreEvent(
        trace_id=uuid.uuid4(),
...
async def test_approval_pipeline_rejection_low_pnl() -> None:
    bus = AsyncMock()
    fsm = AsyncMock()
    pipeline = StrategyApprovalPipeline(bus, fsm, min_pnl=5.0)
    sandbox_report = SandboxReportEvent(
        trace_id=uuid.uuid4(),
        source="S",
        timestamp=1,
...
async def test_approval_pipeline_system_failure() -> None:
    ...
    event = await pipeline.evaluate_strategy(Crasher(), MagicMock())

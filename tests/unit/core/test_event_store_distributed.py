...
@pytest.fixture
def store():
    """Create a temporary event store for isolated unit testing."""
...
async def test_event_idempotency(store):
    """Verify that multiple appends of the same event_id are ignored."""
...
async def test_startup_index_rebuild(store):
    """Verify that the index is successfully rebuilt from disk on restart."""
    ...
    # Simulate restart by creating a new store instance pointing to same path
...
async def test_offset_range_query(store):
    """Verify offset-based range queries work correctly."""
    partition = "ADA"
    for i in range(10):
        evt = MarketEvent(
            trace_id=uuid.uuid4(), 
...
async def test_write_latency_simulation(store):
    """Simulate high load and check write latency (target < 5ms)."""
    import time
    ...
    # Check if average is within reasonable bounds even on shared environment
    # Locally would be <1ms, but we set a safe limit for basic disk

...
def test_orderbook_imbalance_compute_success(execution_config: MagicMock) -> None:
    ...
    assert model.compute(bids, asks) == 0.0  # noqa: S101
    ...
    target_pos = 0.3
    assert imbalance > target_pos  # noqa: S101
    assert imbalance == pytest.approx(0.333333333)  # noqa: S101
    ...
    imbalance = model.compute(bids, asks)
    target_neg = -0.8
    assert imbalance < target_neg  # noqa: S101
...
def test_orderbook_imbalance_weighted_decay(execution_config: MagicMock) -> None:
    ...
    assert imb_a > imb_b  # noqa: S101
    assert imb_b < 0  # noqa: S101
...
def test_orderbook_imbalance_catastrophic_safety(execution_config: MagicMock) -> None:
    ...
    # Empty book
    assert model.compute([], []) == 0.0  # noqa: S101

    # Zero volume
    assert model.compute([[100, 0]], [[101, 0]]) == 0.0  # noqa: S101

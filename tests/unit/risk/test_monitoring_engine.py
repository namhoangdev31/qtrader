...
def engine() -> MonitoringEngine:
    return MonitoringEngine(
        pnl_drift_threshold=500.0, latency_limit_ms=50.0, max_slippage_bps=15.0, min_fill_rate=0.9
    )
...
def test_monitor_pnl_drift(engine: MonitoringEngine) -> None:
    metrics = SystemMetrics(
        pnl_real=1000.0, pnl_expected=1600.0, latency_ms=25.0, slippage_bps=5.0, fill_rate=0.98
    )
    alerts = engine.monitor(metrics)
...
def test_monitor_latency_hard_limit(engine: MonitoringEngine) -> None:
    metrics = SystemMetrics(
        pnl_real=1000.0, pnl_expected=1000.0, latency_ms=65.0, slippage_bps=5.0, fill_rate=0.98
    )
    alerts = engine.monitor(metrics)
...
def test_monitor_execution_degradation(engine: MonitoringEngine) -> None:
    ...
    m2 = SystemMetrics(1000.0, 1000.0, 25.0, 5.0, 0.8)
...
def test_monitor_health_reporting(engine: MonitoringEngine) -> None:
    engine.monitor(SystemMetrics(1000.0, 2000.0, 10.0, 5.0, 0.98))
    engine.monitor(SystemMetrics(1000.0, 1000.0, 100.0, 5.0, 0.98))

...
def test_momentum_alpha():
    ...
    res = alpha.compute(df)
    assert len(res) == 6
    assert res.name == "momentum"
    # The first few values should be null or handled by _zscore
    # Momentum is upwards, so last value should be positive if z-scored
...
def test_mean_reversion_alpha():
    ...
    # Price dropped from 105 to 100, so mean reversion should be positive (rebound)
    assert res.tail(1).item() > 0
...
def test_alpha_look_ahead_bias():
    ...
    alpha = MomentumAlpha(lookback=2, zscore_window=3)
    res_full = alpha.compute(df_full)
    res_partial = alpha.compute(df_partial)

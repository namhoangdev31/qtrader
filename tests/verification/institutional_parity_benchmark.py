...
def benchmark_stats_parity():
    ...
    logger.info(
        f"Latency   | Python: {(end_py - start_py) * 1000:.2f}ms | Rust: {(end_rs - start_rs) * 1000:.2f}ms"
    )
...
def benchmark_risk_latency():
    ...
    order = qtrader_core.Order(
        1,
        "BTC-USD",
        qtrader_core.Side.Buy,
        0.5,
        65000.0,
        qtrader_core.OrderType.Limit,
        int(time.time()),
    )
    iterations = 10000
    ...
    avg_latency_us = (end - start) / iterations * 1000000
    ...
    if avg_latency_us < 100:
        ...
        logger.warning(
            f"RESULT: [FAIL] Latency is {avg_latency_us:.2f}μs - potential system contention."
        )

...
## PHẦN 8: BẢN ĐỒ CHI TIẾT CÁC TIỂU MODULE (GRANULAR SUB-MODULE MAP)

Đây là phân tích sâu tới cấp độ thư mục con và tệp tin trọng yếu, làm cơ sở để AI Agent và Kỹ sư hệ thống có cái nhìn chính xác nhất về "điểm rơi" của dự án QTrader.

### 8.1 Nhóm Thực thi (Execution Granular Audit)

Nhóm này là "gót chân Achilles" của hệ thống với mật độ mã giả (stubs) cao nhất.

| Tiểu Module | Tệp tin trọng yếu | Tiến độ | Ảnh hưởng | Mục tiêu loại bỏ / Tái cấu trúc |
| :--- | :--- | :--- | :--- | :--- |
| **Brokers** | `coinbase.py`, `binance.py` | 70% | High | Hợp nhất các Adapter phân mảnh về `brokers/` chuẩn |
| **Algos** | `vwap.py`, `twap.py`, `pov.py` | 40% | High | Chuyển đổi logic Slicing tĩnh sang Dynamic Forecasting |
| **Routing** | `router.py`, `cost_model.py` | 20% | Critical | Thay thế `0.85` stub bằng mô hình `VenueFillModel` thật |
| **Core Exec** | `execution_engine.py` | 50% | Critical | Lấp đầy 12 phương thức `pass` bằng cơ chế Self-healing |
| **Microsec** | `toxic_flow.py`, `imbalance.py` | 10% | Med | Di chuyển từ mồ côi sang phân tích Alpha Feed |

> [!WARNING]
> Thư mục `execution/microstructure/` hiện hoàn toàn mồ côi (không được import). Nếu AI Agent cố gắng sửa lỗi ở đây sẽ không có tác dụng gì đến hệ thống đang chạy.

### 8.2 Nhóm Dữ liệu (Data Pipeline Granular Audit)

Nhóm này có nền tảng tốt nhưng vi phạm các tiêu chuẩn Zero-Latency.

| Tiểu Module | Tệp tin trọng yếu | Tiến độ | Ảnh hưởng | Mục tiêu loại bỏ / Tái cấu trúc |
| :--- | :--- | :--- | :--- | :--- |
| **Market** | `coinbase_market.py` | 80% | Critical | Loại bỏ `time.sleep(1.0)` trong loop Websocket |
| **Store** | `duckdb_client.py` | 90% | Med | Tích hợp nén dữ liệu cũ (Data Archiving) |
| **Pipeline** | `normalizer.py`, `arbitrator.py` | 60% | High | Chuyển `gap_detector.py` từ stub sang logic phát hiện mất tick |

### 8.3 Nhóm Lõi (Core Framework Granular Audit)

Trái tim của hệ thống cần được làm sạch các phần code "ma".

| Tiểu Module | Tệp tin trọng yếu | Tiến độ | Ảnh hưởng | Mục tiêu loại bỏ / Tái cấu trúc |
| :--- | :--- | :--- | :--- | :--- |
| **Orchestration** | `orchestrator.py` | 50% | Critical | Xóa bỏ thư mục `qtrader/system/` rỗng |
| **Validation** | `precision_validator.py` | 40% | High | Ép buộc sử dụng `math_authority` tại layer DB |
| **Monitoring** | `latency_monitor.py` | 60% | High | Chuyển log tĩnh sang Prometheus/Grafana alerts |
| **EventBus** | `event_bus.py`, `error_bus.py`| 85% | Critical | Loại bỏ `core/async_adapter.py` dư thừa |

### 8.4 Danh sách tệp tin "Độc hại" và Cần tái sinh (Toxic & Legacy List)

Danh sách các file gây nhiễu cho AI Agent và cần được ưu tiên xử lý:

- **Toxic (Cần Xóa/Di chuyển)**:
  - `qtrader/governance/kill_switch.py`: Trùng lặp logic rủi ro.
  - `qtrader/execution/order_fsm.py`: Phá vỡ tính nhất quán của OMS.
  - `qtrader/core/event_validator.py`: Code cũ, chồng chéo với hệ thống Precision mới.
  
- **Legacy (Cần Tái sinh từ `_deprecated/`)**:
  - `tca/slippage.py`: Tool phân tích trượt giá cực mạnh nhưng đang bị bỏ hoang.
  - `hft/spoofing.py`: Nhận diện lệnh ảo, cần thiết cho SOR nâng cao.

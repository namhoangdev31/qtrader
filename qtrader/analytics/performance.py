...
try:
    ...
    HAS_RUST_CORE = False
    logging.warning("qtrader_core (Rust) not found. Falling back to slow Python/Polars implementations.")

class PerformanceAnalytics:
    """Calculates performance and risk metrics using high-performance Rust core when available."""
    ...
    def calculate_metrics(
        ...
        if len(equity_series) < 2:
            return self._empty_metrics(peak=float(equity_series.max()) if len(equity_series) > 0 else 0.0)

        # Convert to numpy/list for Rust ingestion
        equity_vals = equity_series.to_list()
        returns_series = equity_series.pct_change().drop_nulls()
        returns_vals = returns_series.to_list()

        if HAS_RUST_CORE:
            # 1. Core Performance using Rust MathEngine
            max_drawdown, peak_equity = math_engine.calculate_max_drawdown(equity_vals)
            
            # 2. Risk Metrics using Rust StatsEngine (Institutional Grade)
            expected_shortfall = stats_engine.calculate_historical_es(returns_vals, confidence_level)
            omega_ratio = stats_engine.calculate_omega_ratio(returns_vals, self.risk_free_rate / 252.0)
            sortino_ratio = stats_engine.calculate_sortino_ratio(returns_vals, self.risk_free_rate / 252.0, 252.0)
            ...
            # Minimal fallbacks for advanced metrics
    ...
    def _empty_metrics(self, peak: float = 0.0) -> dict[str, float]:
        return {
            ...
            "sortino_ratio": 0.0, "omega_ratio": 0.0,
            "calmar_ratio": 0.0, "expected_shortfall_historical": 0.0,
            "max_drawdown": 0.0, "peak_equity": peak,
            "has_rust_acceleration": float(HAS_RUST_CORE)
        }

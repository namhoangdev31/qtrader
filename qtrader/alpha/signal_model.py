...
class ProbabilisticSignalModel:
    ...
    def predict_proba(self, x_matrix: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
        """
        ...
        return self.model.predict_proba(x_matrix)[:, 1].astype(np.float64)  # type: ignore[no-any-return]

    def compute_expected_return(self, x_matrix: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
        """
        ...
        """
        p_up = self.predict_proba(x_matrix)
        return (p_up * self.mu_up - (1.0 - p_up) * self.mu_down).astype(np.float64)

    def get_signal(
        self, x_matrix: npt.NDArray[np.float64], theta_buy: float = 0.6, theta_sell: float = 0.4
    ) -> npt.NDArray[np.int8]:
        """

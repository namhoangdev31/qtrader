...
class MLAlphaEngine(BaseAlpha):
    ...
    async def generate(self, market_data: MarketData) -> AlphaOutput:
        ...
        return AlphaOutput(
            ...
            alpha_values={
                "ml_signal": 1.0 if decision_action == "BUY" else -1.0 if decision_action == "SELL" else 0.0,
                "ml_confidence": confidence,
                "traditional_signal": 0.0,
                "combined_signal": confidence if decision_action != "HOLD" else 0.0,
                "chronos_trend": "UP" if decision_action == "BUY" else "DOWN" if decision_action == "SELL" else "FLAT",
                "tabpfn_risk": "LOW",

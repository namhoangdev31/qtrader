...
class LiveFeedbackEngine:
    ...
    def __init__(
        ...
        self._execution_metrics: deque[
            tuple[datetime, str, str, float, float | None, float, float, float]
    ...
    async def process_fill(self, fill: Any) -> None:
        ...
        Args:
            fill: FillEvent with attributes:
                ...
                timestamp: datetime
        """
        logger.info(f"DEBUG: process_fill called for {fill.symbol} {fill.side} {fill.price} at {fill.timestamp}")
        try:
            # First, try to match as an opening fill
            matched_signal_idx = self._find_matching_signal_for_entry(fill)
            logger.info(f"DEBUG: Opening fill match result: {matched_signal_idx}")
            ...
            logger.info(f"DEBUG: Exit fill match result: {matched_trade_idx}")
    ...
    def _find_matching_signal_for_entry(self, fill: Any) -> int | None:
        ...
        """
        def _get_strategy_value(obj, attr_name: str = 'strategy') -> str | None:
            # First try metadata
    ...
    def _find_matching_trade_for_exit(self, fill: Any) -> int | None:
    ...
    async def _handle_exit_fill(self, fill: Any, trade_idx: int) -> None:
        ...
        """
        logger.info(f"DEBUG: _handle_exit_fill called for fill: symbol={fill.symbol}, side={fill.side}, price={fill.price}")
        trade = self._open_trades[trade_idx]
        logger.info(f"DEBUG: Trade details: symbol={trade.symbol}, strategy={trade.strategy}, entry_price={trade.entry_price}, size={trade.size}")
...
# import polars as pl
#
# from qtrader.monitoring.feedback.live_feedback_engine import LiveFeedbackEngine
# from qtrader.core.event_bus import EventBus, EventType
#
#

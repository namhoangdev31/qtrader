...
class CoinbaseMarketDataClient:
    ...
    async def get_candles(
        ...
        end_ts = (
            ...
            else int(end or datetime.now(Config.tz).timestamp())
        )
        ...
        while current_end > start_ts:
            ...
            try:
                ...
                # Rate limiting via tenacity retry decorator on _get() — no manual sleep needed
                # The @retry decorator with wait_exponential handles backoff automatically

            except Exception as e:
                _LOG.error(f"Error fetching candle batch at {current_end}: {e}")
                # tenacity retry handles backoff — break to let retry logic handle it
                break
        ...
        df = df.with_columns(
            [
                ...
                pl.col("open")
                .cast(pl.String)
                .map_elements(lambda x: math_authority.d(x), return_dtype=pl.Object)
                .alias("open"),
                pl.col("high")
                .cast(pl.String)
                .map_elements(lambda x: math_authority.d(x), return_dtype=pl.Object)
    ...
    async def get_product_book(self, product_id: str, limit: int = 10) -> dict[str, Any]:
        ...
        return {
            "bids": [
                {
                    "price": math_authority.d(b["price"]),
                    "size": math_authority.d(b.get("size", b.get("volume", 0))),
                }
                for b in pb.get("bids", [])
            ],
            "asks": [
                {
                    "price": math_authority.d(a["price"]),
                    "size": math_authority.d(a.get("size", a.get("volume", 0))),
                }
    ...
    async def get_best_bid_ask(self, product_ids: list[str]) -> dict[str, dict[str, Decimal]]:
        ...
        for pb in pricebooks:
            ...
            bid_size = (
                math_authority.d(bids[0].get("size", bids[0].get("volume", 0)))
                if bids
                else Decimal("0")
            )
            ask_size = (
                math_authority.d(asks[0].get("size", asks[0].get("volume", 0)))
                if asks
                else Decimal("0")
            )

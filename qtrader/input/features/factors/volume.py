...
__all__ = ["OBV", "VWAP", "DollarVolume", "VolumeRatio", "ForceIndex"]
...
class DollarVolume(BaseFeature):
    ...
    def __init__(self, rolling_window: int | None = None) -> None:
        ...
        self.name = f"dollar_volume" if rolling_window is None else f"dollar_volume_{rolling_window}d"
...
class ForceIndex(BaseFeature):
    ...
    def __init__(self, ema_period: int | None = 13) -> None:
        ...
        self.name = f"force_index" if ema_period is None else f"force_index_{ema_period}"
...
def test_obv_increases_on_up_days() -> None:
    import polars as pl
    from qtrader.input.features.factors.volume import OBV
...
def test_vwap_between_low_and_high() -> None:
    import polars as pl
    from qtrader.input.features.factors.volume import VWAP
...
def test_dollar_volume_equals_close_times_volume() -> None:
    import polars as pl
    from qtrader.input.features.factors.volume import DollarVolume

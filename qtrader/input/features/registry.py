...
from qtrader.input.features.base import Feature

__all__ = ["FeatureRegistry"]

_LOG = logging.getLogger("qtrader.input.features.registry")
...
class FeatureRegistry:
    ...
    Pattern mirrors ``qtrader.input.alpha.registry.AlphaRegistry``.
...
def build_default_registry() -> FeatureRegistry:
    ...
    from qtrader.input.features.factors.technical import (
        ATR, MACD, RSI, BollingerBands, MomentumReturn, ROC,
    )
    from qtrader.input.features.factors.volume import (
        DollarVolume, ForceIndex, OBV, VWAP, VolumeRatio,
    )
    from qtrader.input.features.factors.lagged import LaggedReturn, ReturnVolatility
...
def test_registry_get_raises_on_unknown() -> None:
    import pytest
    from qtrader.input.features.registry import FeatureRegistry
...
def test_build_default_registry_has_rsi() -> None:
    from qtrader.input.features.registry import build_default_registry
...
def test_registry_compute_all() -> None:
    import polars as pl
    from qtrader.input.features.factors.technical import RSI
    from qtrader.input.features.registry import FeatureRegistry

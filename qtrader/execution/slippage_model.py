...
class SlippageModel:
    ...
    async def compute_slippage(
        ...
        try:
            ...
            permanent = self.permanent_impact * Decimal(str(math.sqrt(float(participation_rate)))) * mid_price

            # Volatility-adjusted component
            vol_component = self.volatility_factor * volatility * mid_price * Decimal(str(math.sqrt(float(participation_rate))))

            # Random component to simulate market noise
            random_component = Decimal(str(random.gauss(0, float(vol_component) / 2)))
            ...
            if side.upper() == 'SELL':
                slippage = -slippage  # Selling pushes the price down (negative slippage from the trader's perspective)

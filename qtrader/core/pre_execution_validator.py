...
class PreExecutionValidator:
    ...
    def validate(self, seed_manager: Any = None) -> bool:
        ...
        if is_ready:
            logger.success(
                "PRECHECK_SUCCESS | System state is VALID. Ready for orchestrator ignition."
            )
        else:
    ...
    def _check_float_usage(self) -> None:
        ...
        self.results.append(
            ValidationResult(
                "FloatScan",
                not has_high_risk,
                f"Detected {report.get('high_risk')} high-risk float usages"
                if has_high_risk
                else "No high-risk float usage detected",
            )
        )
    ...
    def _generate_reports(self, is_ready: bool) -> None:
        ...
        FloatScanner(str(self.root_path))

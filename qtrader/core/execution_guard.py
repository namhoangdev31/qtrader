...
class ExecutionGateRegistry:
    ...
    def log_blocked(self, module: str, function: str) -> None:
        ...
        logger.critical(f"EXECUTION_GUARD_VIOLATION | Blocked attempt: {module}.{function} | SystemState: {state_manager.state.name}")
...
def require_initialized(func: F) -> F:
    ...
    def wrapper(*args: Any, **kwargs: Any) -> Any:
        ...
        if current_state not in [SystemState.READY, SystemState.RUNNING]:
            ...
            raise RuntimeError(f"Illegal execution attempt: {func.__name__} requires INITIALIZED system state (Current: {current_state.name})")

...
class LifecycleTaskManager:
    """Manages background tasks triggered by system heartbeat events.
    ...
    async def _on_system_event(self, event: Any) -> None:
        """Handle system events, specifically HEARTBEAT."""
    ...
    async def _do_sentiment_refresh(self) -> None:
        """Actually perform the market sentiment refresh."""
        try:
    ...
    async def _do_pnl_record(self, session_id: str) -> None:
        """Actually record PnL snapshots to the database."""
        try:
    ...
    async def _do_health_log(self, session_id: str) -> None:
        """Actually record system health metrics."""
        import os

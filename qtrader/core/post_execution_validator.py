...
class PostExecutionValidator:
    ...
    async def validate(self, event_store: BaseEventStore, state_store: StateStore) -> dict[str, Any]:
        """
        ...
        # 4. Aggregate Results
        is_valid = trace_results["complete"] and state_results["consistent"] and determinism_results["deterministic"]
        total_issues = trace_results["issues"] + state_results["issues"] + determinism_results["issues"]
        ...
        if is_valid:
            logger.success("POST_EXEC_SUCCESS | Execution output verified. Trace is complete and deterministic.")
        else:
            logger.warning(f"POST_EXEC_WARNING | Execution anomalies detected (Issues: {total_issues}). Status: {report['status']}")
    ...
    async def _check_state_consistency(self, state_store: StateStore) -> dict[str, Any]:
        ...
        if state.active_orders:
            issues.append(f"STALE_ACTIVE_ORDERS: {len(state.active_orders)} orders remained registered.")
    ...
    async def _verify_determinism(self, event_store: BaseEventStore) -> dict[str, Any]:
        ...
        if current_metrics["event_count"] != baseline.get("event_count"):
            issues += 1
            mismatches.append(f"Event count mismatch: {current_metrics['event_count']} vs {baseline['event_count']}")

...
class QTraderSettings(BaseSettings):
    ...
    impact_daily_volume: float = 1000000.0
    impact_sigma_daily: float = 0.02
    impact_y: float = 1.0
    ts_max_position_usd: float = 100000.0
    ts_max_drawdown_pct: float = 0.2
    ts_max_order_qty: float = 1.0
    ts_max_order_notional: float = 50000.0
    ...
    def validate_live_mode(self) -> QTraderSettings:
        if not self.simulate_mode:
            if not self.binance_api_key and (not self.coinbase_api_key):
                raise ValueError(
                    "Live mode requires at least one exchange API key. Set BINANCE_API_KEY or COINBASE_API_KEY in .env."
                )
        return self

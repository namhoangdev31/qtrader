...
class TradingSystemConfig:
    ...
    max_position_usd: float = field(
        default_factory=lambda: config_manager.get("position_size_pct") * settings.starting_equity
    )
    max_drawdown_pct: float = field(
        default_factory=lambda: config_manager.get("max_drawdown_limit")
    )
    ...
    max_orders_per_second: float = field(
        default_factory=lambda: config_manager.get("ts_max_orders_per_second")
    )
...
class TradingSystem:
        ...
        self.pre_trade_risk = PreTradeRiskValidator(
            PreTradeRiskConfig(
                ...
                max_position_per_symbol=Decimal(
                    str(self.config.max_position_usd / self.config.reference_price)
                ),
    ...
    async def stop(self) -> None:
        ...
        if self.active_session_id:
            ...
            await self.db_writer.stop_session(
                self.active_session_id, Decimal(str(balance["equity"])), self._stats
            )
            ...
            await self.event_bus.publish(
                SystemEvent(
                    ...
                    payload=SystemPayload(
                        ...
                        metadata={"price": float(price), "symbol": symbol},
                    ),
                )
            )
        ...
        with TraceAuthority.inject_trace():
            ...
            if ml_result:
                ...
                if exit_signal:
                    ...
                    if signal and self.signal_engine.check_trend_confirmation(
                        symbol, signal["side"], self._market_data[symbol]
                    ):
    ...
    async def _get_market_data(self, symbol: str) -> dict[str, Any]:
        quote = (
            self.broker._quotes.get(symbol)

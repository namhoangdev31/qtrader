...
log = logging.getLogger("qtrader.output.analyst.report")
...
class ReportBuilder:
    """Build rich HTML reports from analyst sections (text, tables, charts).
    ...
    Usage::
        ...
        rb = ReportBuilder("My Backtest Report")
        rb.add_text("### Overview", "Strategy: momentum on BTC-USD 1h")
        rb.add_table("Metrics", metrics_df)

        fig, ax = plt.subplots()
        ax.plot(equity_curve)
        ax.set_title("Equity Curve")
        rb.add_figure("Equity Curve", fig)
        plt.close(fig)

        rb.save("report.html")
    """

    # Minimal CSS embedded in report
    _CSS = """
    <style>
...
<body>
  ...
  {body}
    ...
    def save(self, path: str | Path) -> Path:
        """Write report to disk and return the path."""
        p = Path(path)

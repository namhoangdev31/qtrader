...
class FeatureValidator:
    ...
    def validate(
        ...
        for name, series in features.items():
            ...
            # Compute metrics
            ic_mean = ic_series.mean() if not ic_series.is_empty() else 0.0
            ic_std = ic_series.std() if not ic_series.is_empty() else 0.0
            ...
            if is_valid:
                ...
                validated_features[name] = pl.Series(
                    [0.0] * len(series), name=series.name, dtype=pl.Float64
                )
    ...
    def _compute_ic_series(self, feature: pl.Series, forward_returns: pl.Series) -> pl.Series:
        ...
        feature_mean = feature.rolling_mean(window_size=self.ic_lookback)
        feature_std = feature.rolling_std(window_size=self.ic_lookback)
        returns_mean = forward_returns.rolling_mean(window_size=self.ic_lookback)
        returns_std = forward_returns.rolling_std(window_size=self.ic_lookback)
        
        # Calculate rolling covariance
        covariance = (feature - feature_mean) * (forward_returns - returns_mean)
    ...
    def _compute_decay_rate(self, ic_series: pl.Series) -> float:
        ...
        n = len(ic_series)
        x = pl.Series(list(range(n)), dtype=pl.Float64)
        y = ic_series

...
from qtrader.core.events import SystemEvent, SystemPayload
...
class RegimeDetector:
    ...
    def train(
        self, X: pl.DataFrame, y: pl.Series | None = None, params: dict | None = None
    ) -> None:
        """
        ...
        if X.is_empty() or len(X) < self.min_samples:
            _LOG.warning(
                f"Insufficient samples for regime detection: {len(X)} < {self.min_samples}"
            )
            return
    ...
    def partial_update(self, X: pl.DataFrame) -> SystemEvent | None:
        """
        ...
        Returns:
            SystemEvent if regime changed, None otherwise
        """
        if X.is_empty():
            return None
        ...
        if (
            self._sample_count % self.update_frequency == 0
        ...
        regime_changed = (
            ...
            and latest_regime != self._last_regime
            and latest_regime >= 0  # Valid regime
        )
        ...
        if regime_changed:
            ...
            event = SystemEvent(
                ...
                trace_id="regime_change",
                payload=SystemPayload(
                    action="REGIME_CHANGE",
                    reason=f"Regime changed from {self._last_regime} to {latest_regime}",
                    metadata={
            ...
            _LOG.info(
                f"Regime change detected: {self._last_regime} -> {latest_regime} (confidence: {confidence:.3f})"
            )
            return event

...
class ModelEvaluator:
    ...
    def rolling_ic(self, predictions_df: pl.DataFrame, window: int = 30) -> pl.Series:
    ...
    def feature_importance_report(self, model: Any, feature_names: list[str]) -> pl.DataFrame:
        ...
        df = pl.DataFrame({"feature": feature_names, "importance": importances.tolist()}).sort(
    ...
    def backtest_predictions(
        self, df: pl.DataFrame, transaction_cost_bps: float = 10.0
    ) -> dict[str, float]:
...
class NestedCrossValidation:
    def __init__(
        self, outer_pipeline: WalkForwardPipeline, inner_pipeline: WalkForwardPipeline
    ) -> None:
    ...
    def evaluate(
        ...
        for i, (outer_train, outer_test) in enumerate(outer_splits):
            ...
            results.append({"fold": i, "best_params": best_params, "test_ic": test_score})
        return results

    def _score(
        self, model: Any, df: pl.DataFrame, target_col: str, feature_cols: list[str] | None
    ) -> float:
        if target_col not in df.columns:
            raise ValueError(f"Target column '{target_col}' not found in DataFrame.")
...
if __name__ == "__main__":
    ...
    orch.validate()
    _df = pl.DataFrame(
        {"x": [0.0, 1.0, 2.0, 3.0, 4.0], "forward_return": [0.1, 0.2, -0.1, 0.05, 0.0]}
    )
    ...
    _res = _nc.evaluate(
        _df, target_col="forward_return", train_func=_train, param_grid=[{}], feature_cols=["x"]
    )

...
mod tests {
    ...
    fn test_position_partial_close() {
        ...
        assert_eq!(pos.qty, 1.0);
        assert_eq!(pos.avg_entry_price, 50000.0); // Cost basis shouldn't change on reduction
    }
    ...
    fn test_account_equity() {
        ...
        // equity = cash(60000) + pos_value(1.0 * 50000) = 110000
        assert_eq!(account.equity(&prices), 110000.0);
    }
    ...
    fn test_position_full_close_and_flip() {
        ...
        // Cost basis might reset or not depending on implementation, but quantity must be 0

        // Flip to short
        pos.add_fill(Side::Sell, 2.0, 2600.0);
        assert_eq!(pos.qty, -2.0);
        // Cost basis updates to new entry if previous qty was 0

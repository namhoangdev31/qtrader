...
fn test_risk_fat_finger() {
    ...
    let order_qty = Order::new(1, "BTC".to_string(), Side::Buy, 101.0, 50000.0, OrderType::Limit, 1000);
    assert!(core.check_order(&order_qty, &account, 50000.0, 100000.0).is_err());
    let order_notional = Order::new(2, "BTC".to_string(), Side::Buy, 0.5, 50000.0, OrderType::Limit, 1000);
...
fn test_risk_position_limit() {
    ...
    let order_1 = Order::new(1, "BTC".to_string(), Side::Buy, 0.01, 50000.0, OrderType::Limit, 1000);
    assert!(core.check_order(&order_1, &account, 50000.0, 10000.0).is_ok());
    account.add_position_direct("BTC".to_string(), 0.01, 50000.0);
    let order_2 = Order::new(2, "BTC".to_string(), Side::Buy, 0.02, 50000.0, OrderType::Limit, 1000);

      ...
      <div className="flex-1 overflow-y-auto p-1.5 lg:p-2 scroll-smooth scrollbar-thin">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2 border-b border-[#1e222d] pb-2 mb-2 glass px-3 rounded-t-lg">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-0.5 overflow-x-auto pb-0.5 scrollbar-hide">
                ...
                />
                <HeaderBadge
                  icon={<MonitorCheck size={8} />}
                  label="HFT TELEMETRY"
                  color="text-blue-400"
                />
                <HeaderBadge
                  icon={<Activity size={8} />}
                  label="ATOMIC TRIO CORE"
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div
              className={`px-2 py-1 rounded border text-[8px] font-black uppercase tracking-widest flex items-center gap-1.5 ${
                isSessionActive
            ...
            >
              <span
                className={`w-1 h-1 rounded-full ${isSessionActive ? "bg-emerald-400 animate-pulse" : "bg-slate-700"}`}
              />
          ...
          snapshot={
            {
              ...
              current_price:
                portfolio.current_price || (portfolio as any).base_price || 0,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-6">
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="h-[430px]">
              <ForensicNotes notes={audit.notes} baseUrl={getBaseUrl()} />
            </div>
          ...
          <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-6 leading-tight">
            <div className="md:col-span-2 min-h-[150px]">
              <FlowVisualizer
                ...
                lastSignal={
                  forensics?.thinking_history?.[
                    forensics.thinking_history.length - 1
                  ]
                }
        ...
        <footer className="mt-auto py-2 px-4 border-t border-[#1e222d] flex items-center justify-between text-[9px] font-bold text-slate-600 bg-black/40">
          <div className="flex items-center gap-6">
            <span className="flex items-center gap-2 animate-pulse">
              <MonitorCheck size={10} className="text-blue-500" /> REAL-TIME
              FORENSICS: ACTIVE{" "}
            </span>
            <span className="flex items-center gap-2">
              <Zap size={10} className="text-blue-500" /> PEAK EQUITY: $
              {(portfolio?.equity ?? 0).toFixed(2)}
            </span>
            <span className="flex items-center gap-2">
              <TrendingUp size={10} className="text-emerald-500" /> MAX DD:
              0.00%
            </span>
          </div>
          <div className="flex items-center gap-4">
            <span className="tracking-[0.2em] uppercase">Status: 200 OK</span>
            <span className="tracking-[0.2em] uppercase text-blue-500">
              Institutional_v3 (Manual)
            </span>
          </div>
        </footer>
      </div>
...
function HeaderBadge({
  ...
  return (
    <div
      className={`flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest whitespace-nowrap px-2 py-1 bg-black/20 rounded border border-white/5 ${color}`}
    >
      {icon} {label}
    </div>
  );
